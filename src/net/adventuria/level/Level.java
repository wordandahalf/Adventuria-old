...
public class Level {
		...
		if (Mouse.isLeftButtonClicked()) {
			if (!Inventory.isOpen) {
				if (Mouse.getBlockX() < worldW && Mouse.getBlockY() < worldH
						&& Mouse.isBlockInRange()) {
					Block b = this.getBlock(Mouse.getBlockX(),
							Mouse.getBlockY());
						...
						if (b.getID().getHardness() < 128) {
							...
							if (b.getHardness() <= 0) {
								Component.inventory.addItemToInventory(b
										.getID());
								this.setBlock(BlockID.AIR, Mouse.getBlockX(),
										Mouse.getBlockY());
							}
						}
					}
			...
			if (!Inventory.isOpen) {
				if (Mouse.getBlockX() < worldW && Mouse.getBlockY() < worldH
						&& Mouse.isBlockInRange()) {
					if (!(Mouse.getBlockLocation().equals(Component.character
							.getBlockLocation()))
							&& !(Mouse.getBlockLocation().equals(new Location(
									Component.character.getBlockX(),
									Component.character.getBlockY() + 1)))) {
						if (this.getBlock(Mouse.getBlockX(), Mouse.getBlockY())
								.getID() == BlockID.AIR
								|| this.getBlock(Mouse.getBlockX(),
										Mouse.getBlockY()).getID() == BlockID.WATERSOURCE
								&& Mouse.getBlockX() > -1
								&& Mouse.getBlockY() > -1) {
							if (Component.inventory.getHeldItemID() != BlockID.AIR) {
								this.setBlock(
										Component.inventory.getHeldItemID(),
			...
			for (Block b : bArray) {
				if (b.getID() == BlockID.WATERSOURCE) {
					if (this.getBlock(b.getBlockX(), b.getBlockY() + 1).getID() == BlockID.AIR) {
						this.setBlock(BlockID.WATERSOURCE, b.getBlockX(),
								b.getBlockY() + 1);
					} else if (this.getBlock(b.getBlockX() + 1, b.getBlockY())
							.getID() == BlockID.AIR) {
						this.setBlock(BlockID.WATERSOURCE, b.getBlockX() + 1,
								b.getBlockY());
					} else if (this.getBlock(b.getBlockX() - 1, b.getBlockY())
							.getID() == BlockID.AIR) {
						this.setBlock(BlockID.WATERSOURCE, b.getBlockX() - 1,
	...
	public void Render(Graphics g, int camX, int camY, int renW, int renH) {
		for (int x = camX / Block.tileSize; x < camX / Block.tileSize + renW; x++) {
			for (int y = camY / Block.tileSize; y < camY / Block.tileSize
					+ renH; y++) {
				if ((x >= 0) && (y >= 0) && (x < worldW) && (y < worldH)) {
					this.Blocks[x][y].Render(g);
					if ((this.Blocks[x][y]
							.contains(new Point(Mouse.getX()
									/ Component.pixelSize + (int) Component.sX,
									Mouse.getY() / Component.pixelSize
											+ (int) Component.sY)))
							&& (this.Blocks[x][y].getID() != BlockID.AIR)
							&& (!Inventory.isOpen) && Mouse.isBlockInRange())

					{
						g.setColor(new Color(255, 255, 255, 64));
						g.fillRect(this.Blocks[x][y].x - camX,
								this.Blocks[x][y].y - camY,
								this.Blocks[x][y].width,
								this.Blocks[x][y].height);
					}
				}
			}
		}
