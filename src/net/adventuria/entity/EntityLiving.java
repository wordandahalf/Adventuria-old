...
public abstract class EntityLiving extends DoubleRectangle implements EntityBase {
	...
	public boolean isCollidingWithBlock(Location pt1, Location pt2, boolean solid) {
		...
		if ((Component.level.Blocks[x1][y1].getBounds().intersects(getBoundingRectangle()) && Component.level.Blocks[x1][y1].isSolid() == solid) || (Component.level.Blocks[x2][y2].getBounds().intersects(getBoundingRectangle()) && Component.level.Blocks[x2][y2].isSolid() == solid)) {
			return true;
		}
		return false;
	}
	...
	public boolean isCollidingWithBlockID(Location pt1, Location pt2, BlockID id) {
		...
		int y1 = pt1.y >= 0 ? pt1.y : 0;
		int y2 = pt2.y >= 0 ? pt2.y : 0;
		if ((Component.level.Blocks[x1][y1].getBounds().intersects(getBoundingRectangle()) && Component.level.Blocks[x1][y1].getID() == id) || (Component.level.Blocks[x2][y2].getBounds().intersects(getBoundingRectangle()) && Component.level.Blocks[x2][y2].getID() == id)) {
			return true;
		}
		return false;
	}
