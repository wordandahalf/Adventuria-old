...
public class EntityPlayer extends EntityHuman {
	...
	public void Tick() {
		if ((!this.isJumping)
				&& (!isCollidingWithBlock(new Point((int) this.x + 2,
						(int) (this.y + this.height)), new Point((int) (this.x
						+ this.width - 2.0D), (int) (this.y + this.height))))) {
		...
		if (isCollidingWithBlock(new Point((int) this.x + 2,
				(int) (this.y + this.height)), new Point((int) (this.x
				+ this.width - 2.0D), (int) (this.y + this.height)))) {
			if (fallingSpeed > 1.4) {
		...
		if ((Component.isMoving) && (!Inventory.isOpen)) {
			...
			if (Component.dir == this.movementSpeed) {
				canMove = isCollidingWithBlock(new Point(
						(int) (this.x + this.width), (int) this.y), new Point(
						(int) (this.x + this.width),
						(int) (this.y + (this.height - 2.0D))));
			} else if (Component.dir == -this.movementSpeed) {
				canMove = isCollidingWithBlock(new Point((int) this.x - 1,
						(int) this.y), new Point((int) this.x - 1,
						(int) (this.y + this.height - 2.0D)));
			}
		...
		if (this.isJumping) {
			if (!isCollidingWithBlock(new Point((int) (this.x + 2.0D),
					(int) this.y), new Point(
					(int) (this.x - this.width - 2.0D), (int) this.y))) {
	...
	public boolean isCollidingWithBlock(Point pt1, Point pt2) {
		for (int x = (int) (this.x / Block.tileSize); x < (int) (this.x / Block.tileSize) + 3; x++) {
			for (int y = (int) (this.y / Block.tileSize); y < (int) (this.y / Block.tileSize) + 3; y++) {
				if ((x >= 0) && (y >= 0) && (x < Component.level.Blocks.length)
						&& (y < Component.level.Blocks[0].length)) {
					if ((Component.level.Blocks[x][y].getID().isSolid())) {
						if ((Component.level.Blocks[x][y].contains(pt1))
								|| (Component.level.Blocks[x][y].contains(pt2))) {
							return true;
						}
					}
				}
	...
	public void Render(Graphics g) {
		if (Component.dir == this.movementSpeed) {
			g.drawImage(AssetManager.tileset_entity,
					(int) (this.x - Component.sX),
					(int) (this.y - Component.sY),
					(int) ((this.width + this.x) - Component.sX),
					(int) ((this.height + this.y) - Component.sY),
					EntityID.PLAYER.getTextureID()[0] * Block.tileSize
							+ Block.tileSize * this.animation,
					EntityID.PLAYER.getTextureID()[1] * Block.tileSize,
					EntityID.PLAYER.getTextureID()[0] * Block.tileSize
							+ Block.tileSize * this.animation
							+ (int) this.width,
					EntityID.PLAYER.getTextureID()[1] * Block.tileSize
							+ (int) this.height, null);
		} else {
			g.drawImage(AssetManager.tileset_entity,
					(int) (this.x - Component.sX),
					(int) (this.y - Component.sY),
